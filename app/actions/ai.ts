...
export async function generateBouquetImage(
  ...
  // Build prompt via the single shared builder — the same function the client
  // preview uses, so "Показать prompt" cannot drift from what is sent here.
  const enhancedPrompt = buildBouquetImagePrompt(selectedItems, visualizationParams)
