-- ─────────────────────────────────────────────────────────────────────────
-- migration_025_add_sale_price_to_purchase_atomic_rpc.sql
-- Исправление бага: sale_price не писался в inventory_items.
--
-- Единственное изменение по сравнению с migration_011:
--   В INSERT INTO inventory_items добавлены колонка sale_price
--   и значение v_sale_price.
--   Колонка inventory_items.sale_price уже существует (migration_004).
--
-- Логика функции не меняется:
--   - flowers.sale_price по-прежнему обновляется (шаг 7d)
--   - inventory_items.sale_price теперь тоже записывается (шаг 7a)
--   - backfill старых записей не выполняется
--
-- Зависит от: migration_011_add_variety_to_purchase_atomic_rpc.sql
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- ИЗМЕНЕНИЕ 025: добавлен sale_price / v_sale_price
