-- ─────────────────────────────────────────────────────────────────────────
-- migration_009_create_purchase_atomic_rpc.sql
-- Атомарное создание поставки в одной PostgreSQL-транзакции.
--
-- Зависит от:
--   migration_001_init.sql   (purchases, suppliers, get_user_organization_id)
--   migration_004_inventory_schema.sql (inventory_items, stock_movements, purchase_items)
--
...
-- ════════════════════════════════════════════════════════════
-- RPC: create_purchase_atomic
--
-- Атомарно выполняет:
--   1. Проверяет организацию через get_user_organization_id()
--      (organization_id никогда не принимается от клиента).
--   2. Находит поставщика по имени (case-insensitive) или создаёт нового.
--   3. Создаёт запись purchases.
--   4. Для каждой позиции из p_items:
--      a. Проверяет, что flower_id существует и принадлежит организации.
--      b. Вычисляет effective_cost = cost_price + delivery_per_unit.
--      c. Создаёт партию в inventory_items.
--      d. Записывает движение 'purchase' в stock_movements.
--      e. Создаёт позицию в purchase_items.
--      f. Обновляет flowers.sale_price, если передана ненулевая цена.
--   5. Возвращает {"purchase_id": "<uuid>"}.
--
-- Если любой шаг падает — PostgreSQL откатывает всю транзакцию.
--
-- Параметры:
--   p_supplier_name  text     — имя поставщика (обязательно, не пустое)
--   p_supplier_phone text     — телефон (опционально, только для новых поставщиков)
--   p_purchase_date  date     — дата поставки (DEFAULT CURRENT_DATE)
--   p_comment        text     — комментарий к поставке (опционально)
--   p_delivery_cost  numeric  — общая стоимость доставки (DEFAULT 0)
--   p_items          jsonb    — массив позиций (обязательно, не пустой)
--
-- Формат p_items (массив объектов):
--   flower_id  uuid     — обязательно
--   quantity   int      — обязательно, > 0
--   cost_price numeric  — обязательно, >= 0 (закупочная без доставки)
--   sale_price numeric  — опционально, > 0 (обновит flowers.sale_price)
--   expires_at date     — опционально (срок годности партии)
--   comment    text     — опционально
--
-- Delivery split:
--   delivery_per_unit = p_delivery_cost / sum(quantity)
--   effective_cost    = cost_price + delivery_per_unit  (→ inventory_items.cost_price)
--   extra_costs       = round(delivery_per_unit * quantity * 100) / 100  (→ purchase_items.extra_costs)
--
-- Возвращает:
--   {"purchase_id": "<uuid>"}
-- При любой ошибке — RAISE EXCEPTION, транзакция откатывается.
-- ════════════════════════════════════════════════════════════
...
BEGIN
  ...
  --    organization_id не принимается от клиента ни в каком виде.
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Цветок должен существовать и принадлежать организации
    SELECT EXISTS (
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    INSERT INTO stock_movements (
      ...
      v_org_id,
      v_flower_id,
      v_inventory_id,    -- NOT NULL: обязательно указываем
      v_quantity,        -- положительное — приход
      'purchase',
      'purchase',
      v_purchase_id,
      v_item_comment,
    ...
    INSERT INTO purchase_items (
      ...
      inventory_item_id,
      quantity,
      cost_price,        -- закупочная без доставки
      extra_costs,       -- доля доставки за эту строку
      expires_at,
      comment
    ) VALUES (
      v_purchase_id,
...
-- ════════════════════════════════════════════════════════════
-- ПРАВА НА ВЫПОЛНЕНИЕ
-- Запретить всем через PUBLIC и anon.
-- Разрешить только authenticated-пользователям.
-- ════════════════════════════════════════════════════════════
