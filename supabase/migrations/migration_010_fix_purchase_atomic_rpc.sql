-- ─────────────────────────────────────────────────────────────────────────
-- migration_010_fix_purchase_atomic_rpc.sql
-- Исправление бага в create_purchase_atomic:
--   в INSERT INTO inventory_items отсутствовало значение v_purchase_id.
--   Добавлена строка 'v_purchase_id' между 'fresh' и v_user_id в VALUES.
--
-- Зависит от: migration_009_create_purchase_atomic_rpc.sql
-- Идемпотентна: CREATE OR REPLACE FUNCTION.
-- НЕ меняет RLS, схему таблиц, политики, существующие данные.
-- ─────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    INSERT INTO inventory_items (
      ...
      freshness_status, purchase_id, created_by
    ) VALUES (
      v_org_id, v_flower_id, v_supplier_id, p_purchase_date,
      v_effective_cost, v_quantity, v_quantity, v_expires_at,
      'fresh', v_purchase_id, v_user_id        -- исправлено: добавлен v_purchase_id
    )
